...
class MotorControl:
    ...
    def __init__(self, device='COM29'):
    ...
    def TwoBytesTo16Int(self, high_byte: int, lo_byte: int):
        combined = ((high_byte & 0xFF) << 8) | (lo_byte & 0xFF)
        if combined >= 0x8000:  # tanda bit 16 menyala, artinya bilangan negatif (signed int16)
            combined -= 0x10000
        return combined
    ...
    def read_reply(self, _id, timeout=0.01):
            ...
            if len(res) != 0:
                ...
                ## size of ring_buffer not 10, and still some place to fill
                    ...
                    if len(ring_buffer) == 10:
                        crc_read = ring_buffer[9]
                        crc_calc = self.crc8(bytes(ring_buffer[0:9]))
        ...
        if len(ring_buffer) == 10:
            rpm = self.TwoBytesTo16Int(ring_buffer[2], ring_buffer[3])
            current = self.TwoBytesTo16Int(ring_buffer[4], ring_buffer[5])
            return rpm, current, ring_buffer[8]
        else:
            return self.prev_fb_rpm[_id-1], self.prev_fb_cur[_id-1], 0
