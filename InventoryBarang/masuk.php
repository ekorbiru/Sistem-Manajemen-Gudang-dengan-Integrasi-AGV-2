...
<head>
    ...
    <title>AWG Inventory - Barang Masuk</title>
                        ...
                        <div class="card-body">
                            <table id="datatablesSimple">
                                ...
                                <tbody>
                                    ...
                                    $query = "SELECT riwayatmasuk.*, masukbarang.namabarang 
                                              FROM riwayatmasuk 
                                              JOIN masukbarang ON riwayatmasuk.idbarang = masukbarang.idbarang 
                                              WHERE 1=1";
                                    ...
                                    if ($tanggal != '') {
                                        $query .= " AND DATE(tanggalmasuk) = '$tanggal'";
                                    }
                                    if ($lokasi != '') {
                                        $query .= " AND riwayatmasuk.lokasi LIKE '%$lokasi%'";
                                    }
            ...
            <div class="modal-content">
                ...
                <form method="post">
                    <div class="modal-body">
                        ...
                        <br>
                        <?php
                        // Koneksi sudah ada dari file require 'function.php'

                        // 1. Ambil semua lokasi yang sudah dipakai
                        $lokasi_terisi = [];
                        $cek_lokasi = mysqli_query($conn, "SELECT DISTINCT lokasi FROM riwayatmasuk");
                        while ($row = mysqli_fetch_assoc($cek_lokasi)) {
                            $lokasi_terisi[] = $row['lokasi'];
                        }

                        // 2. Daftar lokasi tetap
                        $daftar_lokasi = ['Lemari A', 'Lemari B', 'Lemari C', 'Lemari D'];
                        ?>
                        ...
                        <select name="lokasi" class="form-control" required>
                            ...
                            <?php foreach ($daftar_lokasi as $lokasi): ?>
                                <?php
                                $terisi = in_array($lokasi, $lokasi_terisi);
                                $tulisan = $terisi ? "$lokasi (Terisi)" : $lokasi;
                                ?>
                                <option value="<?= $lokasi ?>"><?= $tulisan ?></option>
                            <?php endforeach; ?>
                        </select>
                        <br>
